...
function login(){
    if (isset($_POST['email']) && isset($_POST['pass']) ) {
        if ($_POST['email'] != "" && $_POST['pass'] != "") {
            if (strlen($_POST['email'])>49 or strlen(md5($_POST['pass']))>199) {
                header('Location: index.php?login_error=length_error', true, 303);
            }else{
...
function signup() : void {
    if (isset($_POST['nom']) && isset($_POST['email']) && isset($_POST['pass'])) {
        if ($_POST['nom'] != "" && $_POST['email'] != "" && $_POST['pass'] != "" ) {
            if (strlen($_POST['nom'])>24 or strlen($_POST['email'])>49 or strlen(md5($_POST['pass']))>199 ) {
                header('Location: index.php?reg_error=length_error', true, 303);
            }else{
